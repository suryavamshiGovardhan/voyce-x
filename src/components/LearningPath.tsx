...
const learningStages: (Stage & {
  ...
  { level: "L5", name: "The Architect", description: "You are both the sculptor and the sculpture, the architect and the building.", deepContent: "Active reconstruction begins. You learn to consciously rewire thought patterns, build new neural pathways, and architect your internal world. Somatic practices integrate body wisdom as you become the deliberate creator of your consciousness.", tools: ["Neural Rewiring", "Somatic Integration", "Conscious Creation"], duration: "8-10 weeks", icon: Microscope, color: "text-primary", bgColor: "bg-orange-600", borderColor: "border-border", hoverBorder: "hover:border-border", japaneseIcon: "torii" },
  ...
  { level: "L7", name: "The Void / Godhead", description: "In the end, there is no end. There is no ego to kill, no self to find. There is only this.", deepContent: "The final dissolution—or perhaps the beginning. Philosophy, mysticism, and the direct experience of non-dual awareness. Integration of all previous work into a lived understanding that transcends concepts and enters pure being.", tools: ["Non-Dual Inquiry", "Mystical Integration", "Being Practices"], duration: "Lifetime Practice", icon: Crown, color: "text-primary", bgColor: "bg-orange-600", borderColor: "border-border", hoverBorder: "hover:border-border", japaneseIcon: "lotus" },
];
    ...
    <div className="space-y-8">
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <span className="text-foreground text-sm">Your Journey Progress</span>
          {user ? (
            <span className="text-primary text-sm">
              {progressLoading ? "…" : `${percent}% complete · ${completed.size}/${learningStages.length} stages`}
            </span>
          ) : (
            <Link to="/login" className="text-primary text-sm underline hover:text-primary">
              Sign in to track progress
            </Link>
          )}
        </div>
        <Progress value={user ? percent : 0} className="h-3 bg-muted" />
            ...
            >
              <CardContent className="p-6">
                ...
                <div className="flex items-center mb-4">
                  ...
                  <h3 className="text-foreground font-medium text-lg">{stage.name}</h3>
                </div>

                <p className="text-muted-foreground text-sm leading-relaxed mb-4 italic">"{stage.description}"</p>

                <div className="mb-4">
                  <p className="text-muted-foreground text-xs leading-relaxed mb-3">{stage.deepContent}</p>

                  <div className="mb-3">
                    <span className="text-primary text-xs font-medium">Core Tools:</span>
                    <div className="flex flex-wrap gap-1 mt-1">
                      {stage.tools.map((tool) => (
                        <span key={tool} className="bg-muted text-muted-foreground text-xs px-2 py-1 rounded">
                          {tool}
                        </span>
                      ))}
                    </div>
                ...
                <Button
                  size="sm"
                  className={`w-full haptic-btn text-sm ${unlocked ? `${stage.bgColor} hover:opacity-90 text-foreground` : 'bg-muted text-muted-foreground cursor-not-allowed'}`}
      ...
      <div className="mt-12 text-center">
        <div className="bg-black/30 backdrop-blur-sm border border-border rounded-lg p-6">
          <h3 className="text-foreground text-xl font-light mb-3">The Journey Never Ends</h3>
          <p className="text-muted-foreground text-sm leading-relaxed italic max-w-2xl mx-auto">
